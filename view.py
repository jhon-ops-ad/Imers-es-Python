...
def desativar_conta(id):
    with Session(engine) as session:
        ...
        conta = session.exec(statement).first() #devolve direto, pois a fk é unica
        if conta.valor > 0: #verifica se tem saldo
...
def movimentar_dinheiro(historico: Historico):
    with Session(engine) as session:
        ...
        #TODO: VERIFICAR SE A CONTA ESTA ATIVA
...
def criar_grafico_por_conta():
    with Session(engine) as session:
        ...
        contas = session.exec(statement).all()
        bancos = [i.banco.value for i in contas] #list comprehension
        total = [i.valor for i in contas] #list comprehension
        import matplotlib.pyplot as plt
        plt.bar(bancos, total) #eixos x e y
        #plt.show()
        # bancos = []
        # for i in contas:
...
# criar_conta(conta)
#desativar_conta(1)
#transferir_saldo(3, 1, 5)
# historico = Historico(conta_id=1, tipos=Tipos.ENTRADA, valor=15, data=date.today())
